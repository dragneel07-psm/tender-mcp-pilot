...
from tender_monitor import queries, storage


class SecurityTests(unittest.TestCase):
    def test_public_government_url_is_accepted(self):
        source = storage.validate_source({
        ...
        for url in ("http://127.0.0.1", "http://localhost", "http://10.0.0.1"):
            with self.subTest(url=url):
                with self.assertRaises(ValueError):
                    storage.validate_source({"name": "Unsafe", "url": url, "notice_url": url})

    def test_invalid_notice_limit_is_rejected(self):
        with self.assertRaises(ValueError):
            queries.list_notices(limit="invalid")

    def test_limit_is_bounded(self):
        self.assertIsInstance(queries.list_notices(limit=500), list)
